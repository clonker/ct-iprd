...
TEMPLATE_TEST_CASE("Test Integration", "[integration]", ctiprd::systems::DoubleWell<float>, ctiprd::systems::LotkaVolterra<float>) {
    ...
    TestType system {};
    auto pool = ctiprd::config::make_pool(10);
    auto integrator = ctiprd::cpu::integrator::EulerMaruyama{system, pool};
    integrator.particles()->initializeParticles(5000, system.types[0].name);
    integrator.particles()->initializeParticles(5000, system.types[1].name);
